...
import './App.css'
import { Game, GamesResponse, Profile, Team } from './types/Types';
...
function App() {
	...
	const gamesIdsRef = useRef<number[]>([]);
	...
	useEffect(() => {
		if (
			homeTeam?.length &&
			oppTeam?.length &&
			homeTeam?.length === oppTeam?.length
		) {
		...
		fetch(`https://aoe4world.com/api/v0/players/${userId}/games?page=${page}`)
			...
			.then((data: GamesResponse) => {
				if (!isLatestGameSet.current) {
					...
					const lastGame = onGoingGame?.game_id ? onGoingGame : data?.games[0];
							...
							if (oppTeamIds?.includes(player.profile_id)) {
								if (!gamesIdsRef.current.includes(game.game_id)) {
									game?.teams.map((team) => {
										...
										game.teams.sort(() => (isHomeTeam ? -1 : 1));
									});
